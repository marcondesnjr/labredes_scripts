...
tableFormatter <- function(WORKBOOK, dataFrame, sheetName, numStyle, percStyle, lb, hb) {
  ...
  for (col in cols) {
    conditionalFormatting(WORKBOOK, sheetName,
      cols = col, rows = 1:rowsLength + 1,
...
deaCalculator <- function(dataDEA, sh, inputVars = INPUT_VARS, outputVars = OUTPUT_VARS) {
  ...
  dea.plot(inputs, outputs, RTS = "crs", ORIENTATION = "in", main = paste(sh, "CRS_I"))
  dea.plot(inputs, outputs, RTS = "crs", ORIENTATION = "out", main = paste(sh, "CRS_O"))
  dea.plot(inputs, outputs, RTS = "crs", ORIENTATION = "in-out", main = paste(sh, "CRS_O"))
  dea.plot.frontier(inputs, outputs, RTS = "crs", main = paste(sh, "Frontier"))

  results <- list(SCCR_I = SCCR_I, CCR_I = CCR_I, BCC_I = BCC_I, CCR_O = CCR_O, BCC_O = BCC_O)
  return(results)
}
...
processWorksheet <- function(data, sh, inputVars, outputVars, workbookOutput) {
  ...
  CCR_O <- allDEA$CCR_O
  CCR_O$eff <- 1 / CCR_O$eff

  BCC_O <- allDEA$BCC_O
  BCC_O$eff <- 1 / BCC_O$eff
  ...
  effDataframe <- data.frame(SCCR_I$eff)
  colnames(effDataframe) <- c("SCCR_I")
  rownames(effDataframe) <- rownames(dataDEA)

  compDataframe <- cbind(dataDEA, effDataframe)
  sheetName <- paste(sh, "MAIN", sep = ".")
  compDataframe <- compDataframe[order(-compDataframe$SCCR_I), ]
